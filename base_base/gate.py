"""
CAPS Permission Gate for Base Base
...
def has_capability(user=None, capability=None):
    """Check if user has a specific Base Base capability via CAPS."""
    if not user:
    ...
    if not capability.startswith("BB_"):
        capability = f"BB_{capability}"
