...
def seed_data():
    ...
    """
    frappe.logger().info("Base Base: Running seed_data()...")

    _seed_settings()
    _seed_roles()

    frappe.db.commit()
    frappe.logger().info("Base Base: seed_data() complete.")
...
def _seed_settings():
    ...
    settings_dt = None
    for dt_name in ["BB Settings", "Base Base Settings", "Base_Base Settings"]:
...
def _seed_roles():
    ...
    roles = [
        {"role": "BB User", "desk_access": 1},
        {"role": "BB Manager", "desk_access": 1},
        {"role": "BB Admin", "desk_access": 1},
    ]
