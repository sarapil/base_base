# Contributing to Base Base — المساهمة في Base Base
...
```bash
git clone https://github.com/YOUR_USERNAME/base_base.git
cd base_base
```
...
```bash
bench get-app base_base /path/to/your/fork
bench --site dev.localhost install-app base_base
...
bench --site dev.localhost run-tests --app base_base --module base_base.tests.unit.test_my_service
```
...
- Update help files in `base_base/help/` for DocType changes
